...
{
	if(!FileExists("scripts\\vscripts\\IT-KILLER\\ammoboxfixer.nut")) {
		SetFailState("The vscript file does not exist: %s","scripts\\vscripts\\IT-KILLER\\ammoboxfixer.nut");
	}
	if(!FileExists("addons\\stripper\\global_filters.cfg")) {
		SetFailState("The Stripper:Source file does not exist: %s","addons\\stripper\\global_filters.cfg");
	}
	...
	HookEntityOutput("game_player_equip", "OnUser4", Ammo);
...
public void Ammo(char[] output, int caller, int activator, float delay)
{
	if(!activator || !sm_ammobox_fix_enabled.BoolValue) return;
	RefillWeapon(activator);
	if(sm_ammobox_fix_extrarefill.FloatValue!=0.0) {
		CreateTimer(sm_ammobox_fix_extrarefill.FloatValue, Timer_ExtraRefill, activator, TIMER_FLAG_NO_MAPCHANGE);
	}
}
